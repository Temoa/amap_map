...
class CircleUpdates {
  /// 通过Circle的前后更新集合构造一个CircleUpdates
  CircleUpdates.from(Set<Circle> previous, Set<Circle> current) {
    ...
    final Set<String> _circleIdsToRemove = prevCircleIds.difference(currentCircleIds);
    ...
    final Set<Circle> _circlesToChange = currentCircleIds.intersection(prevCircleIds).map(idToCurrentCircle).where(hasChanged).toSet();
  ...
  bool operator ==(Object other) {
    ...
    return setEquals(circlesToAdd, typedOther.circlesToAdd) && setEquals(circleIdsToRemove, typedOther.circleIdsToRemove) && setEquals(circlesToChange, typedOther.circlesToChange);
  }

  @override
  int get hashCode => Object.hashAll([circlesToAdd, circleIdsToRemove, circlesToChange]);
